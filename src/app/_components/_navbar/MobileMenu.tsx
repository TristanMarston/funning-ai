...
const MenuItems = ({ links, isOpen, setIsOpen }: { links: NavLink[]; isOpen: boolean; setIsOpen: React.Dispatch<React.SetStateAction<boolean>> }) => {
    return (
        <nav className='block navbar-md:hidden fixed top-0 bottom-0 right-0 w-full min-h-full translate-y-[-100%] will-change-transform z-[50] overflow-y-auto px-5 pt-26 navbar-sm:pt-28'>
            <div className='overflow-y-auto p-6 pb-4 navbar-sm:p-8 navbar-sm:pb-5 h-auto rounded-3xl navbar-sm:rounded-4xl mobile-menu-gradient-border w-full max-w-max-width'>
                <ul className='flex flex-col items-start gap-4'>
                    {links.map(({ name, href }) => (
                        <li
                            key={name}
                            className='text-[22px] leading-[1.4] navbar-sm:text-2xl w-full font-figtree font-semibold text-primary hover:text-primary/80 transition-colors duration-200'
                        >
